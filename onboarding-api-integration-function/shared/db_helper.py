...
def get_last_timestamp(db_config):
    ...
    query = "SELECT TOP 1 last_timestamp FROM dbo.onboarding_watermark ORDER BY ID DESC"
...
def get_onboarding_data(db_config,last_ts):

    conn = get_db_connection(db_config)
    cursor = conn.cursor()

    cursor.execute("EXEC dbo.usp_GetOnboardingData @LastTimestamp = ?", last_ts)
