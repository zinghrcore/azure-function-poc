...
def get_last_timestamp(db_config):
    ...
    query = "SELECT TOP 1 last_timestamp FROM dbo.salaryrevision_watermark ORDER BY ID DESC"
...
def update_last_timestamp(db_config,new_timestamp):
    ...
    query = """
    Update dbo.salaryrevision_watermark 
    set last_timestamp = ?
    """
...
def get_salaryrevision_data(db_config,last_ts):

    conn = get_db_connection(db_config)
    cursor = conn.cursor()

    cursor.execute("EXEC dbo.usp_GetsalaryrevisionData @LastTimestamp = ?", last_ts)
...
def log_batch(db_config,batch_size, status, details,IsBatchComplete,Payload):
    ...
    cursor = conn.cursor()
    query = """
    INSERT INTO dbo.salaryrevision_Log
