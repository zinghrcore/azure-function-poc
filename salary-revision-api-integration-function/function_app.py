...
from shared.db_helper import (
    get_last_timestamp,
    get_salaryrevision_data,
    update_last_timestamp,
    log_batch
)

from shared.queue_helper import send_to_queue
from shared.api_helper import call_salaryrevision_api
...
def salaryrevision_timer(myTimer: func.TimerRequest):

    logging.info("========== SALARY REVISION TIMER STARTED ==========")
    ...
    try:
        ...
        records = get_salaryrevision_data(last_ts)

        if not records:
            logging.info("No new salaryrevision records found")
            return
    ...
    logging.info("========== SALARY REVISION TIMER FINISHED ==========")
...
@app.queue_trigger(
    arg_name="azqueue",
    queue_name="salaryrevision-queue",
    connection="AzureWebJobsStorage"
)
def process_salaryrevision_batch(azqueue: func.QueueMessage):

    logging.info("========== SALARY REVISION QUEUE STARTED ==========")
    ...
    try:
        ...
        records = transform_records(records, COUNTRY)
        ...
        # Step 4: Call API
        status_code, response = call_salaryrevision_api(final_payload)
    ...
    logging.info("========== SALARY REVISION QUEUE FINISHED ==========")
